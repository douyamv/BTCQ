...
def from_remote(args):
    ...
    print(f"  总供应:            {info['total_supply']/1e8:,.4f} BTCQ")
    print(f"  总抵押:            {info['total_stake']/1e8:,.4f} BTCQ")
    print(f"  Mempool:           {info.get('mempool_size', '?')} 笔待打包")
    print(f"  Peer 数:           {len(info.get('peers', []))}")
    print(f"  罚没:              {info.get('slash_count', 0)} 条 / {info.get('total_slashed', 0)/1e8:,.4f} BTCQ")
    print()
    end = info['height']
