"""转账交易（账户模型，类 Ethereum 风格）。

v0.1 设计：
* from / to / amount / nonce / signature 五字段
...
class Transaction:
    ...
    def unsigned_bytes(self) -> bytes:
        ...
        return b"".join([
            self.sender,
            self.recipient,
            self.amount.to_bytes(16, "big"),    # 最多 2^128，远超总量
...
def sign_transaction(wallet: Wallet, recipient_bytes: bytes, amount: int, nonce: int) -> Transaction:
