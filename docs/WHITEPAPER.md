...
```
1. 出块人通过 VRF 由抵押权重抽签产生
2. 出块人用 prev_hash + height + 自身地址派生确定性电路 seed
3. 出块人在量子机上运行 RCS 电路（n=24, depth=8, 1024 shots, 约 1 秒）
4. 计算样本的 XEB 分数，必须 ≥ 当前难度阈值
5. 出块人签名并广播区块
6. 验证者：经典模拟同一电路（≈ 1 秒），核对 XEB
```

**为什么这设计可行**：

* **量子门槛真实存在**：n=24 + depth=8 的电路需要 256 MB 状态向量、约 5–10 秒经典模拟才能"伪造"出合格 XEB。**1 秒块时间内经典做不到**——只有量子机能在期限内提交合规证明。
* **抵押决定话语权**：抵押越多，被抽中出块概率越大，对应的奖励也越多
* **罚没机制**：提交无效证明的抵押者将被罚没（v0.5 完整实现）
